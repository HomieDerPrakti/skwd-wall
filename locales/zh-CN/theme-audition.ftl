...
settings-playback-target-dialog-title = 自动暂停目标
settings-playback-target-dialog-desc = 添加正在运行的进程，以便在其打开时暂停动态壁纸。移除不应再暂停壁纸的目标。
settings-playback-targets = 目标
settings-playback-targets-empty = 尚未配置进程目标。
settings-playback-running = 正在运行的进程
settings-playback-running-empty = 没有返回正在运行的进程。
settings-playback-target-add = 添加
settings-playback-target-remove = 移除
settings-playback-target-close = 关闭
settings-playback-target-action-desc = 查看暂停壁纸的目标，并添加正在运行的进程。
settings-playback-target-manual-placeholder = 进程名称，例如 mpv
